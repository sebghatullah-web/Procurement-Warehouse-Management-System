...
<div class="filter-box rounded-2 p-3 mb-3">
  <form method="get" class="row g-2 align-items-end">
    <div class="col-md-4"><label class="form-label small text-muted">جستجو</label>
      <input type="text" name="q" class="form-control form-control-sm" value="<?php echo h($q); ?>" placeholder="کالا / موقعیت"></div>
    ...
    <div class="col-md-2 pt-4">
      <div class="form-check"><input class="form-check-input" type="checkbox" name="low" value="1" id="lowOnly" <?php echo $low?'checked':''; ?> onchange="this.form.submit()">
        <label class="form-check-label form-label" for="lowOnly">فقط کالاهای کم‌موجود</label></div>
    </div>
    <div class="col-auto pt-3"><button class="btn btn-primary btn-sm" type="submit">فیلتر</button></div>
    <div class="col-auto pt-3"><a class="btn btn-outline-secondary btn-sm" href="<?php echo $base; ?>/reports/inventory.php">بازنشانی</a></div>
  </form>
</div>

<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    <span><i class="bi bi-archive me-2"></i>موجودی انبار <span class="text-muted small">(<?php echo count($rows); ?> رکورد)</span></span>
    <button class="btn btn-sm btn-outline-secondary" data-print><i class="bi bi-printer me-1"></i>چاپ</button>
  </div>
  <div class="table-responsive">
    <table class="table table-hover align-middle mb-0">
      <thead><tr><th>کالا</th><th>دسته‌بندی</th><th>تعداد</th><th>واحد</th><th>حداقل موجودی</th><th>موقعیت</th><th>به‌روزرسانی</th></tr></thead>
      <tbody>
      ...
      <?php endforeach; ?>
      <?php if (count($rows) === 0): ?>
        <tr><td colspan="7" class="text-center text-muted py-4">رکوردی در موجودی نیست.</td></tr>
      <?php endif; ?>
      </tbody>
    </table>
  </div>
